...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(value = "select sum(o.total_price) from sub_gadgets s " +
                   ...
                   "where o.status ilike 'DELIVERED' or o.status ilike 'RECEIVED' and o.created_at = current_date",
            nativeQuery = true)
    BigDecimal getBuyPrice();


    @Query(value = "select count(o.id) from orders o " +
                   "where (o.status ilike 'DELIVERED' or o.status ilike 'RECEIVED') and date(o.created_at) = current_date",
            nativeQuery = true)
    int getBuyCount();
    ...
    @Query(value = "select sum(o.total_price) from sub_gadgets s " +
                   ...
                   "join orders o on o.id = og.orders_id " +
                   "where o.status not ilike 'DELIVERED' and o.status not ilike 'CANCELED' " +
                   "and o.status not ilike 'RECEIVED' and o.created_at = current_date",
            nativeQuery = true)
    BigDecimal getOrderPrice();


    @Query(value = "select count(o.id) from orders o " +
                   "where (o.status ilike 'DELIVERED' or o.status ilike 'RECEIVED') " +
                   "and o.status != 'CANCELLED' and date(o.created_at) = current_date",
            nativeQuery = true)
    ...
    @Query(value = "select sum(o.total_price) from sub_gadgets s " +
                   ...
                   "where o.status ilike 'DELIVERED' and cast(o.created_at as date) = current_date",
            nativeQuery = true)
    ...
    @Query(value = "select sum(o.total_price) from sub_gadgets s " +
                   ...
                   "inner join orders_sub_gadgets og on og.sub_gadgets_id = s.id " +
                   "inner join orders o on o.id = og.orders_id " +
                   "where o.status ilike 'DELIVERED' and cast(o.created_at as date) = (current_date - interval '1 day')",
            nativeQuery = true)
    ...
    @Query(value = "select sum(o.total_price) from sub_gadgets s inner join gadgets g on s.gadget_id = g.id inner join orders_sub_gadgets og on og.sub_gadgets_id = s.id inner join orders o on o.id = og.orders_id where o.status ilike 'DELIVERED' and date_trunc('year', o.created_at) = date_trunc('year', (current_date - interval '1 year'));", nativeQuery = true)
